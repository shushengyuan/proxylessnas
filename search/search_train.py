...
def search_func(args):
    ...
    if args.resume:
        if not os.path.isdir(args.path):
            raise ValueError('resume path does not exist: %s' % args.path)
        resume_log = os.path.join(args.path, 'resume_search_parameters.txt')
    ...
    if args.target_hardware is None:
        ...
        if args.grad_reg_loss_type != 'add#linear_abs':
            hw = ref_values[args.target_hardware]
            key = args.channel_num
            if key not in hw:
                try:
                    key = '%.2f' % float(key)
                except (TypeError, ValueError):
                    pass
            args.ref_value = hw[key]
...
def retrain_func(args, search_log):
    ...
    if os.path.isfile(run_config_path):
        ...
        run_config.label_smoothing = args.label_smoothing
    ...
    if os.path.isfile(net_config_path):
        ...
        # Retrain-only hook (search/inference never reach this block).
        apply_final_conv_prior_bias(net, getattr(args, 'final_prior_pi', -1.0))
        init_ckpt = getattr(args, 'retrain_init_ckpt', '')
        if init_ckpt:
            checkpoint = torch.load(init_ckpt, map_location='cpu')
            state_dict = checkpoint.get('state_dict', checkpoint)
            net.load_state_dict(state_dict)
            print('Loaded retrain init checkpoint: %s' % init_ckpt)
...
def inference(args, search_log, inference_path):
    ...
    if os.path.isfile(net_config_path):
        ...
        checkpoint   = torch.load(
            inference_path + '/checkpoint' + '/model_best.pth.tar',
