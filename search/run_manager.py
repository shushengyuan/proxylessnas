...
from ruamel_yaml import  YAML
# import yaml
from search.utils.utils import middle_block_latency_encoder, middle_block_latency_decoder, middle_block_latency_decoder_half
...
class RunConfig:
    ...
    def _calc_learning_rate(self, epoch, batch=0, nBatch=None, fixed_lr=None):
        if  self.lr_schedule_type == 'cosine':
            ...
            lr      = self.init_lr if fixed_lr is None else fixed_lr
        else:
    ...
    def data_provider(self):
        if self._data_provider is None:
            if self.dataset == 'ucm':
                from data_providers.ucm      import UcmDataProvider
                self._data_provider = UcmDataProvider(**self.data_config)
            if  'SIRST' in self.dataset:
                from data_providers.SIRST    import SIRST_DataProvider
...
class RunManager:
    ...
    def __init__(self, path, net, run_config: RunConfig, out_log=True, crop_size=None,
        ...
        if torch.cuda.is_available():
            ...
            ## CPU fallback for checkpoint audit / inference on CPU-only environments.
    ...
    def compute_latency(self, target_hardware, conv_name, model_name, iterations, fast, random_choose):
        def load_latency_yaml(yaml_path, message='load_exist_yaml'):
            print(message, yaml_path)
            metric_txt = {}
            initial_metric = {}
            operation_name = []
            operation_value = []
            ...
            for id in range(0, len(operation_name)):
                ...
                if str(iterations-iteration)+"_"+str(layer)+'_' in operation_name[id]:
                    cost_list.append(operation_value[id].split('value:')[1])
                else:
                    if len(cost_list)>5:
                        initial_metric['MixedEdge_' + str(iterations - iteration) + "_" + str(layer)] = cost_list
                    else:
            ...
            return operation_name, operation_value, initial_metric, metric_txt

        forced_latency_yaml = os.environ.get(
            'PROXYLESSNAS_LATENCY_%s_YAML' % target_hardware.upper().replace('-', '_')
        ) or os.environ.get('PROXYLESSNAS_LATENCY_YAML')
        if forced_latency_yaml:
            if not os.path.isfile(forced_latency_yaml):
                raise FileNotFoundError(
                    'PROXYLESSNAS latency yaml not found: %s' % forced_latency_yaml
                )
            _, _, initial_metric, metric_txt = load_latency_yaml(forced_latency_yaml, 'load_forced_yaml')
        ...
        for i in range(len(Ready_list)):
            ...
            if ('search' in self.mode) and (self.gene.split('/phase1_gene.txt')[0][-8:] in Ready_list[i]) \
                ...
                operation_name, operation_value, initial_metric, metric_txt = load_latency_yaml(ori_path+'/'+Ready_list[i])
    ...
    def net_flops(self):
        ...
        with torch.no_grad():
            flop, total_params, _ = net.to(self.device).get_flops(input_var)
    ...
    def load_model(self, model_fname=None):
        ...
        try:
            ...
            target_net = self.net.module if hasattr(self.net, 'module') else self.net
        ...
        with torch.no_grad():
            ...
            for i, (images, labels, ori_img, img_id) in enumerate(tbar):
                ...
                if args.postprocess == 'crf':
                    ...
                    output   = torch.from_numpy(np.argmax(prob_seg, axis=0)).type(torch.float32).to(self.device)
    ...
    def inference_latency(self, args, is_test=True, use_train_mode=False):
        net  = self.net
        # DataParallel does not support `.cpu()` on the wrapper; time CPU on the inner module.
    ...
    def train(self, fixed_lr=None):
        ...
        for epoch in range(self.start_epoch, self.run_config.n_epochs):
            ...
            if (epoch + 1) % self.run_config.validation_frequency == 0:
                ...
                val_log = 'Valid [{0}/{1}]\tloss {2:.4f}\tValidate_IoU {3:.4f} ({4:.4f})'.\
