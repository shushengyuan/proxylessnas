...
class ProxylessNASNets(MyNetwork):
    ...
    def build_from_config(config, gene_path):
        ...
        more_down        = config.get('more_down', 'False')
        ...
        if model_name == 'Super_all':
            for i in range(len(config['blocks'])):
                ...
                elif config['blocks'][i]['name'] == 'ShuffleConvLayer':
                    conv_blocks_all = ShuffleConvLayer.build_from_config(config['blocks'][i])
                    block = Block(config['post_transform_conv_block']['in_channels'], config['post_transform_conv_block']['in_channels'], conv_blocks_all=conv_blocks_all)
                    blocks.append(block)

                elif config['blocks'][i]['name'] == 'GhostConvLayer':
                    conv_blocks_all = GhostConvLayer.build_from_config(config['blocks'][i])
                    block = Block(config['post_transform_conv_block']['in_channels'], config['post_transform_conv_block']['in_channels'], conv_blocks_all=conv_blocks_all)
                    blocks.append(block)
                elif config['blocks'][i]['name'] == 'DartsPoolLayer':
                    conv_blocks_all = DartsPoolLayer.build_from_config(config['blocks'][i])
                    block = Block(config['post_transform_conv_block']['in_channels'], config['post_transform_conv_block']['in_channels'], conv_blocks_all=conv_blocks_all)
                    blocks.append(block)
                elif config['blocks'][i]['name'] == 'DartsSkipConnectLayer':
                    conv_blocks_all = DartsSkipConnectLayer.build_from_config(config['blocks'][i])
                    block = Block(config['post_transform_conv_block']['in_channels'], config['post_transform_conv_block']['in_channels'], conv_blocks_all=conv_blocks_all)
                    blocks.append(block)
                elif config['blocks'][i]['name'] == 'DartsSepConvLayer':
                    conv_blocks_all = DartsSepConvLayer.build_from_config(config['blocks'][i])
                    block = Block(config['post_transform_conv_block']['in_channels'], config['post_transform_conv_block']['in_channels'], conv_blocks_all=conv_blocks_all)
                    blocks.append(block)
                elif config['blocks'][i]['name'] == 'DartsDilConvLayer':
                    conv_blocks_all = DartsDilConvLayer.build_from_config(config['blocks'][i])
                    block = Block(config['post_transform_conv_block']['in_channels'], config['post_transform_conv_block']['in_channels'], conv_blocks_all=conv_blocks_all)
                    blocks.append(block)
            ...
            for iteration in range(iterations):
                for layer in range(iterations - iteration):
                    if iteration == 0:
                        ...
                        elif config['blocks'][layer]['name'] == 'DartsPoolLayer':
                            conv_blocks.append(DartsPoolLayer.build_from_config(config['blocks'][layer]))
                        elif config['blocks'][layer]['name'] == 'DartsSkipConnectLayer':
                            conv_blocks.append(DartsSkipConnectLayer.build_from_config(config['blocks'][layer]))
                        elif config['blocks'][layer]['name'] == 'DartsSepConvLayer':
                            conv_blocks.append(DartsSepConvLayer.build_from_config(config['blocks'][layer]))
                        elif config['blocks'][layer]['name'] == 'DartsDilConvLayer':
                            conv_blocks.append(DartsDilConvLayer.build_from_config(config['blocks'][layer]))
                    if   config['blocks'][layer]['name'] == 'ResConvLayer':
                        ...
                        conv_blocks_all.append(SpaConvLayer.build_from_config(config['blocks'][layer]))
                         ...
                         conv_blocks_all.append(DartsPoolLayer.build_from_config(config['blocks'][layer]))
                    elif config['blocks'][layer]['name'] == 'DartsSkipConnectLayer':
                         conv_blocks_all.append(DartsSkipConnectLayer.build_from_config(config['blocks'][layer]))
                    elif config['blocks'][layer]['name'] == 'DartsSepConvLayer':
                         conv_blocks_all.append(DartsSepConvLayer.build_from_config(config['blocks'][layer]))
                    elif config['blocks'][layer]['name'] == 'DartsDilConvLayer':
                         conv_blocks_all.append(DartsDilConvLayer.build_from_config(config['blocks'][layer]))
