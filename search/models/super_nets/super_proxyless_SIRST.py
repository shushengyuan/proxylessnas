...
def _edge_conv_candidates(conv_candidates, count_all):
    """Per-edge op name list: `random_choose` uses dict iter_*; plain list reuses the same names on every edge."""
    if isinstance(conv_candidates, dict):
        return conv_candidates['iter_' + str(count_all)]
    ...
    def __init__(self, in_channel, out_channel, conv=None, conv_type=None, skip_pre=None, skipped=False, iteration=None, mode=None, conv_blocks_all=None):
        ...
        if self.mode == 'Super_half_search':
            for m in range(len(self.operator.candidate_ops)):
                ...
                elif self.operator.candidate_ops[m].config['name'] == 'ShuffleConvLayer':
                    for i in range(len(self.operator.candidate_ops[m].conv_block)):
                        dst = self.operator.candidate_ops[m].conv_block[i]
                        src = conv.candidate_ops[m].conv_block[i]
                        if dst.branch1 is not None:
                            dst.branch1.dw_conv = src.branch1.dw_conv
                            dst.branch1.pw_conv = src.branch1.pw_conv
                        dst.branch2.pw1_conv = src.branch2.pw1_conv
                        dst.branch2.dw_conv  = src.branch2.dw_conv
                        dst.branch2.pw2_conv = src.branch2.pw2_conv

                elif self.operator.candidate_ops[m].config['name'] == 'GhostConvLayer':
                    for i in range(len(self.operator.candidate_ops[m].conv_block)):
                        src, dst = conv.candidate_ops[m].conv_block[i], self.operator.candidate_ops[m].conv_block[i]
            ...
            elif self.operator.config['name'] == 'ShuffleConvLayer':
                for i in range(len(self.operator.conv_block)):
                    dst = self.operator.conv_block[i]
                    src = conv.conv_block[i]
                    if dst.branch1 is not None:
                        dst.branch1.dw_conv = src.branch1.dw_conv
                        dst.branch1.pw_conv = src.branch1.pw_conv
                    dst.branch2.pw1_conv = src.branch2.pw1_conv
            ...
            elif self.operator.config['name'] == 'GhostConvLayer':
                for i in range(len(self.operator.conv_block)):
                    src, dst = conv.conv_block[i], self.operator.conv_block[i]
                    dst.load_state_dict(src.state_dict())
...
class SuperProxylessNASNets(ProxylessNASNets):
    ...
    def __init__(self, conv_type, iterations, backbone, in_channel, conv_candidates,
        ...
        for iteration in range(self.iterations):
            ...
            for layer in range(self.iterations - iteration):
                ...
                if iteration == 0:
                    enc_convs.append(MixedEdge(candidate_ops=build_candidate_ops(_edge_conv_candidates(conv_candidates, count_all), input_channel,
                ...
                enc_convs_all.append(MixedEdge(candidate_ops=build_candidate_ops(_edge_conv_candidates(conv_candidates, count_all), input_channel,
    ...
    def load_param(self, backbone, channel_size, iter):
        ...
        # train_parameters.txt / decode_gene 常为 16、32、48；命令行多为 one/two/… 或 all_32
        cs = channel_size
        if cs is None:
            cs = 'two'
        elif not isinstance(cs, str):
            cs = str(cs)
        cs = cs.strip()
        if cs in ('16', '16.0'):
            channel_size = 'all_16'
        ...
        if    channel_size == 'one':
            ...
            raise ValueError(
                'unknown channel_size %r; use one/two/three/four, all_16/all_32/all_48, or 16/32/48 from train_parameters.txt'
                % (channel_size,)
            )
    ...
    @property
    def searchable_modules(self):
        return [
            block.operator for block in self.encoders
    ...
    def unused_modules_off(self):
        ...
        for m in self.searchable_modules:
            unused = {}
            if MixedEdge.MODE in ['full', 'two', 'full_v2', 'darts']:
    ...
    def unused_modules_back(self):
        if self._unused_modules is None:
            return
        for m, unused in zip(self.searchable_modules, self._unused_modules):
            for i in unused:
