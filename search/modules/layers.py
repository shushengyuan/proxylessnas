...
from search.models.darts_sirst import (
    PoolBN as DartsPoolBNModule,
    SepConv as DartsSepConvModule,
    DilConv as DartsDilConvModule,
    Identity as DartsIdentityModule,
    FactorizedReduce as DartsFactorizedReduceModule,
    _count_executed_module_flops as darts_count_executed_module_flops,
)


def resolve_valid_groups(num_channels, groups):
    if groups is None or groups <= 1 or num_channels <= 1:
        return 1
    return math.gcd(num_channels, min(groups, num_channels))


def channel_shuffle(x, groups):
    b, c, h, w = x.size()
    groups = resolve_valid_groups(c, groups)
    if groups <= 1:
        return x
...
class ShuffleNetV2Block(nn.Module):
    ...
    stride=1 and in_ch == out_ch:
        channel split -> identity | conv branch -> concat -> channel shuffle
    stride>1 or in_ch != out_ch:
        projection branch | conv branch -> concat -> channel shuffle

    When expand_ratio > 1 the conv branch widens internally
    (inverted-bottleneck style within the branch).
    """

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, expand_ratio=1, shuffle_groups=2):
        super(ShuffleNetV2Block, self).__init__()
        self.stride = stride
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.shuffle_groups = resolve_valid_groups(out_channels, shuffle_groups)

        assert out_channels % 2 == 0, 'ShuffleNetV2Block requires even out_channels'
        branch_ch = out_channels // 2
        mid_ch = max(branch_ch, branch_ch * expand_ratio)
        pad = get_same_padding(kernel_size)

        self._use_split = (stride == 1 and in_channels == out_channels)

        if not self._use_split:
            self.branch1 = nn.Sequential(OrderedDict([
                ('dw_conv', nn.Conv2d(in_channels, in_channels, kernel_size, stride, pad,
                                      groups=in_channels, bias=False)),
                ('dw_bn', nn.BatchNorm2d(in_channels)),
                ('pw_conv', nn.Conv2d(in_channels, branch_ch, 1, 1, 0, bias=False)),
                ('pw_bn', nn.BatchNorm2d(branch_ch)),
                ('pw_act', nn.ReLU(inplace=True)),
            ]))
        else:
            self.branch1 = None

        inp = in_channels if not self._use_split else branch_ch

        self.branch2 = nn.Sequential(OrderedDict([
            ('pw1_conv', nn.Conv2d(inp, mid_ch, 1, 1, 0, bias=False)),
            ('pw1_bn', nn.BatchNorm2d(mid_ch)),
            ('pw1_act', nn.ReLU(inplace=True)),
            ('dw_conv', nn.Conv2d(mid_ch, mid_ch, kernel_size, stride, pad,
                                  groups=mid_ch, bias=False)),
            ('dw_bn', nn.BatchNorm2d(mid_ch)),
            ('pw2_conv', nn.Conv2d(mid_ch, branch_ch, 1, 1, 0, bias=False)),
            ('pw2_bn', nn.BatchNorm2d(branch_ch)),
            ('pw2_act', nn.ReLU(inplace=True)),
        ]))
    ...
    def forward(self, x):
        ...
        return channel_shuffle(out, self.shuffle_groups)


class GhostModule(nn.Module):
    """Ghost Module: primary 1x1 conv + cheap depthwise ops, concat to target channels.
    Reference: GhostNet (CVPR 2020) by Kai Han et al."""

    def __init__(self, inp, oup, kernel_size=1, dw_size=3, stride=1, ratio=2, relu=True):
...
class SqueezeExcite(nn.Module):
    """Squeeze-and-excitation used by GhostNet bottlenecks."""

    def __init__(self, channels, se_ratio=0.25):
        super(SqueezeExcite, self).__init__()
        reduced_channels = max(1, int(channels * se_ratio))
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.reduce_conv = nn.Conv2d(channels, reduced_channels, 1, 1, 0, bias=True)
        self.act = nn.ReLU(inplace=True)
        self.expand_conv = nn.Conv2d(reduced_channels, channels, 1, 1, 0, bias=True)
        self.gate = nn.Sigmoid()

    def forward(self, x):
        scale = self.avg_pool(x)
        scale = self.reduce_conv(scale)
        scale = self.act(scale)
        scale = self.expand_conv(scale)
...
class GhostBottleneck(nn.Module):
    ...
    GhostModule(expand, relu) -> [DW Conv if stride>1] -> [SE] -> GhostModule(project, linear) + shortcut.
    Reference: GhostNet (CVPR 2020) by Kai Han et al."""

    def __init__(self, in_channels, out_channels, mid_channels, kernel_size=3,
                 stride=1, ghost_ratio=2, se_ratio=0.0):
        super(GhostBottleneck, self).__init__()
        self.stride = stride
        pad = get_same_padding(kernel_size)

        self.ghost1 = GhostModule(in_channels, mid_channels, kernel_size=1,
                                  dw_size=kernel_size, ratio=ghost_ratio, relu=True)

        if stride > 1:
            self.dw_conv = nn.Sequential(
                nn.Conv2d(mid_channels, mid_channels, kernel_size, stride, pad,
        ...
        self.se = SqueezeExcite(mid_channels, se_ratio) if se_ratio and se_ratio > 0 else None

        self.ghost2 = GhostModule(mid_channels, out_channels, kernel_size=1,
                                  dw_size=kernel_size, ratio=ghost_ratio, relu=False)

        if stride == 1 and in_channels == out_channels:
            self.shortcut = None
        else:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_channels, in_channels, kernel_size, stride, pad,
                          groups=in_channels, bias=False),
...
def set_layer_from_config(layer_config):
    ...
    name2layer = {
        ...
        ShuffleConvLayer.__name__:    ShuffleConvLayer,
        GhostConvLayer.__name__:      GhostConvLayer,
        ZeroLayer.__name__:           ZeroLayer,
        ResConvLayer.__name__:        ResConvLayer,
        DartsPoolLayer.__name__:      DartsPoolLayer,
        DartsSkipConnectLayer.__name__: DartsSkipConnectLayer,
        DartsSepConvLayer.__name__:   DartsSepConvLayer,
        DartsDilConvLayer.__name__:   DartsDilConvLayer,
...
class ShuffleConvLayer(MyModule):
    """ShuffleNet v2 conv layer: channel split/concat + channel shuffle."""

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
                 expand_ratio=1, num_blocks=1, mid_channels=None, shuffle_groups=None):
        super(ShuffleConvLayer, self).__init__()
        ...
        self.expand_ratio = expand_ratio
        self.num_blocks = num_blocks
        self.shuffle_groups = 2 if shuffle_groups is None else shuffle_groups

        layers = []
        layers.append(ShuffleNetV2Block(
            in_channels, out_channels, kernel_size, stride, expand_ratio,
            shuffle_groups=self.shuffle_groups,
        ))
        for _ in range(num_blocks - 1):
            layers.append(ShuffleNetV2Block(
                out_channels, out_channels, kernel_size, 1, expand_ratio,
                shuffle_groups=self.shuffle_groups,
            ))
    ...
    def module_str(self):
        suffix = '_g%d' % self.shuffle_groups if self.shuffle_groups > 1 else ''
        if self.expand_ratio <= 1:
            return '%dx%d_ShuffleV2%s' % (self.kernel_size, self.kernel_size, suffix)
        return '%dx%d_ShuffleMB%d%s' % (self.kernel_size, self.kernel_size, self.expand_ratio, suffix)

    @property
    def config(self):
        return {
            'name': ShuffleConvLayer.__name__,
            'in_channels': self.in_channels,
            'out_channels': self.out_channels,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
    ...
    @staticmethod
    def build_from_config(config):
        return ShuffleConvLayer(
            config['in_channels'], config['out_channels'],
            config['kernel_size'], config.get('stride', 1),
            config.get('expand_ratio', 1), config.get('num_blocks', 1),
            shuffle_groups=config.get('shuffle_groups', 2))
...
class GhostConvLayer(MyModule):
    """GhostNet bottleneck layer: two Ghost Modules + optional DW stride + residual."""

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, expand_ratio=6, num_blocks=1, mid_channels=None, ghost_ratio=2, se_ratio=0.0):
        ...
        layers = []
        layers.append(GhostBottleneck(in_channels, out_channels, feature_dim,
                                      kernel_size, stride, ghost_ratio, se_ratio))
        for _ in range(num_blocks - 1):
            mid = out_channels if expand_ratio == 1 else round(out_channels * expand_ratio)
            layers.append(GhostBottleneck(out_channels, out_channels, mid,
                                          kernel_size, 1, ghost_ratio, se_ratio))
        self.conv_block = nn.Sequential(*layers)
    ...
    def module_str(self):
        suffix = '_se' if self.se_ratio and self.se_ratio > 0 else ''
        if self.expand_ratio == 1:
            return '%dx%d_GhostDepthSC_r%d%s' % (self.kernel_size, self.kernel_size, self.ghost_ratio, suffix)
        return '%dx%d_GhostMB%d_r%d%s' % (self.kernel_size, self.kernel_size, self.expand_ratio, self.ghost_ratio, suffix)

    @property
    def config(self):
        return {
            'name': GhostConvLayer.__name__,
            'in_channels': self.in_channels,
            'out_channels': self.out_channels,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
    ...
    @staticmethod
    def build_from_config(config):
        return GhostConvLayer(
            config['in_channels'], config['out_channels'], config['kernel_size'], config['stride'],
            config['expand_ratio'], config['num_blocks'], config.get('mid_channels'),
            config.get('ghost_ratio', 2), config.get('se_ratio', 0.0))
...
class DartsPoolLayer(MyModule):
    def __init__(self, in_channels, out_channels, pool_type='avg', kernel_size=3, stride=1, padding=1):
        super(DartsPoolLayer, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.pool_type = pool_type
        self.kernel_size = kernel_size
        ...
        if in_channels == out_channels:
            self.op = DartsPoolBNModule(pool_type, out_channels, kernel_size, stride, padding)
        else:
            if pool_type == 'max':
    ...
    def module_str(self):
        return '%s_pool_%dx%d' % (self.pool_type, self.kernel_size, self.kernel_size)

    @property
    def config(self):
        return {
            'name': DartsPoolLayer.__name__,
            'in_channels': self.in_channels,
            'out_channels': self.out_channels,
            'pool_type': self.pool_type,
            'kernel_size': self.kernel_size,
    ...
    def build_from_config(config):
        ...
        return DartsPoolLayer(**config)

    def get_flops(self, x):
        flop, params, y = darts_count_executed_module_flops(self, x)
        return flop, params, y
...
class DartsSkipConnectLayer(MyModule):
    def __init__(self, in_channels, out_channels, stride=1):
        super(DartsSkipConnectLayer, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.stride = stride

        if stride == 1 and in_channels == out_channels:
            self.op = DartsIdentityModule()
        elif stride == 2 and out_channels % 2 == 0:
            self.op = DartsFactorizedReduceModule(in_channels, out_channels)
        else:
            self.op = nn.Sequential(
    ...
    def config(self):
        return {
            'name': DartsSkipConnectLayer.__name__,
            'in_channels': self.in_channels,
            'out_channels': self.out_channels,
            'stride': self.stride,
        }
    ...
    def build_from_config(config):
        config = dict(config)
        config.pop('name', None)
        return DartsSkipConnectLayer(**config)
...
class DartsSepConvLayer(MyModule):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1):
        super(DartsSepConvLayer, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
        self.op = DartsSepConvModule(in_channels, out_channels, kernel_size, stride, padding)

    def forward(self, x):
        return self.op(x)

    def module_str(self):
        return 'sep_conv_%dx%d' % (self.kernel_size, self.kernel_size)

    @property
    def config(self):
        return {
            'name': DartsSepConvLayer.__name__,
            'in_channels': self.in_channels,
            'out_channels': self.out_channels,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
    ...
    def build_from_config(config):
        config = dict(config)
        config.pop('name', None)
        return DartsSepConvLayer(**config)
...
class DartsDilConvLayer(MyModule):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=2, dilation=2):
        ...
        self.padding = padding
        self.dilation = dilation
        self.op = DartsDilConvModule(in_channels, out_channels, kernel_size, stride, padding, dilation)

    def forward(self, x):
        return self.op(x)

    def module_str(self):
        return 'dil_conv_%dx%d' % (self.kernel_size, self.kernel_size)

    @property
    def config(self):
        return {
            'name': DartsDilConvLayer.__name__,
            'in_channels': self.in_channels,
            'out_channels': self.out_channels,
            'kernel_size': self.kernel_size,
            'stride': self.stride,
    ...
    def build_from_config(config):
        config = dict(config)
        config.pop('name', None)
        return DartsDilConvLayer(**config)
