...
class UcmDataProvider(DataProvider):
    ...
    def __init__(self, save_path=None, train_batch_size=256, test_batch_size=512, valid_size=None,
        ...
        if valid_size is not None:
            ...
            train_indexes, valid_indexes = self.random_sample_valid_set(
                [cls for _, cls in train_dataset.samples], valid_size, self.n_classes,)  ### 从Train-set的路径里切出一部分作为valid
            train_sampler = torch.utils.data.sampler.SubsetRandomSampler(train_indexes)
            valid_sampler = torch.utils.data.sampler.SubsetRandomSampler(valid_indexes)

            valid_dataset = datasets.ImageFolder(self.valid_path, transforms.Compose([
    ...
    def save_path(self):
        if self._save_path is None:
            ...
            self._save_path = '/media/gfkd/sda/Dataset/ucm'

        return self._save_path

    @property
    def data_url(self):
        raise ValueError('unable to download Ucm')

    @property
    def train_path(self):
        # return os.path.join(self.save_path, 'train')
        return os.path.join(self.save_path, 'full')
