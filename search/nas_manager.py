...
from search.utils.utils import SoftIoULoss
...
class ArchSearchRunManager:
    ...
    def _use_darts_mode(self):
        return getattr(self.arch_search_config, 'binary_mode', None) == 'darts'

    def _bn_buffer_names(self):
        return ('running_mean', 'running_var', 'num_batches_tracked')

    def _capture_bn_buffers(self):
        buffers = {}
        for name, buffer in self.net.named_buffers():
            if name.endswith(self._bn_buffer_names()):
                buffers[name] = buffer.detach().clone()
        return buffers

    def _restore_bn_buffers(self, buffers):
        if not buffers:
            return
        with torch.no_grad():
            for name, buffer in self.net.named_buffers():
    ...
    def _forward_loss_with_mode(self, images, labels, binary_mode):
        MixedEdge.MODE = binary_mode
    ...
    def _expected_hardware_value(self):
        if self.arch_search_config.target_hardware is None:
            return None
        if self.arch_search_config.target_hardware == 'mobile':
            return self.net.expected_latency(self.run_manager.latency_estimator)
        if self.arch_search_config.target_hardware == 'flops':
            return self.net.expected_flops(self.run_manager.latency_estimator)
        if self.arch_search_config.target_hardware == 'params':
            return self.net.expected_params(self.run_manager.latency_estimator)
        if self.arch_search_config.target_hardware in ['cpu', 'gpu', 'edge-cpu', 'edge-gpu']:
            ...
            return self.net.expected_cpu_gpu_latency(
                self.run_manager,
                self.run_manager.latency_estimator,
                input_var,
                device_type=self.arch_search_config.target_hardware,
            )
        raise NotImplementedError

    def _optimizer_virtual_step(self, weight_params, gradients, lr):
        optimizer = self.run_manager.optimizer
        opt_name = optimizer.__class__.__name__.lower()
        group = optimizer.param_groups[0]

        with torch.no_grad():
            if opt_name == 'adagrad':
                lr_decay = group.get('lr_decay', 0)
                eps = group.get('eps', 1e-10)
                weight_decay = group.get('weight_decay', 0)
                initial_accumulator_value = group.get('initial_accumulator_value', 0)
                for param, grad in zip(weight_params, gradients):
                    if grad is None:
                        continue
                    update = grad
                    if weight_decay != 0:
                    ...
                    if torch.is_tensor(step):
                        step = step.item()
                    step += 1
                    state_sum = state.get('sum', None)
                    if state_sum is None:
                        state_sum = torch.full_like(param, initial_accumulator_value)
                    else:
                        state_sum = state_sum.detach()
                    clr = lr / (1 + (step - 1) * lr_decay)
                    std = (state_sum + update * update).sqrt().add(eps)
                    param.addcdiv_(update, std, value=-clr)
                return
            ...
            if opt_name == 'sgd':
                ...
                for param, grad in zip(weight_params, gradients):
                    ...
                    if weight_decay != 0:
                        update = update + weight_decay * param
                    if momentum != 0:
                        buf = optimizer.state.get(param, {}).get('momentum_buffer', None)
                        if buf is None:
                            buf = update
                        else:
                            buf = momentum * buf + (1 - dampening) * update
                        update = update + momentum * buf if nesterov else buf
    ...
    def _hessian_vector_product(self, train_batch, weight_params, vector, bn_buffers):
        arch_params = list(self.net.architecture_parameters())
        used_vector = [v for v in vector if v is not None]
        if not used_vector:
            return [torch.zeros_like(alpha) for alpha in arch_params]
        vector_norm = torch.cat([v.reshape(-1) for v in used_vector]).norm()
        if vector_norm.item() == 0:
            return [torch.zeros_like(alpha) for alpha in arch_params]

        eps = 0.01 / max(vector_norm.item(), 1e-8)
        train_images, train_labels = train_batch

        with torch.no_grad():
            for param, grad in zip(weight_params, vector):
                if grad is not None:
                    param.add_(grad, alpha=eps)
        loss = self._forward_loss_with_mode(train_images, train_labels, 'darts')
        dalpha_pos = torch.autograd.grad(loss, arch_params, allow_unused=True)
        self._restore_bn_buffers(bn_buffers)

        with torch.no_grad():
            for param, grad in zip(weight_params, vector):
                if grad is not None:
                    param.add_(grad, alpha=-2 * eps)
        loss = self._forward_loss_with_mode(train_images, train_labels, 'darts')
        dalpha_neg = torch.autograd.grad(loss, arch_params, allow_unused=True)
        self._restore_bn_buffers(bn_buffers)
        ...
        hessian = []
        for alpha, pos, neg in zip(arch_params, dalpha_pos, dalpha_neg):
            if pos is None or neg is None:
                hessian.append(torch.zeros_like(alpha))
            else:
                hessian.append((pos - neg) / (2 * eps))
        return hessian

    def _gradient_step_darts(self, train_batch, current_lr):
        if train_batch is None:
            train_batch = self.run_manager.run_config.train_next_batch
        train_images, train_labels = train_batch
        train_images = train_images.to(self.run_manager.device)
        train_labels = train_labels.to(self.run_manager.device)

        val_images, val_labels = self.run_manager.run_config.valid_next_batch
        val_images = val_images.to(self.run_manager.device)
        val_labels = val_labels.to(self.run_manager.device)

        if current_lr is None:
            current_lr = self.run_manager.optimizer.param_groups[0]['lr']

        self.run_manager.net.train()
        bn_buffers = self._capture_bn_buffers()
        weight_params = list(self.net.weight_parameters())
        arch_params = list(self.net.architecture_parameters())
        original_weights = [param.detach().clone() for param in weight_params]

        expected_value = None
        try:
            train_loss = self._forward_loss_with_mode(train_images, train_labels, 'darts')
            train_grads = torch.autograd.grad(train_loss, weight_params, allow_unused=True)
            self._restore_bn_buffers(bn_buffers)

            with torch.no_grad():
                self._optimizer_virtual_step(weight_params, train_grads, current_lr)

            val_loss = self._forward_loss_with_mode(val_images, val_labels, 'darts')
            v_grads = torch.autograd.grad(val_loss, arch_params + weight_params, allow_unused=True)
            self._restore_bn_buffers(bn_buffers)
            dalpha = []
            for alpha, grad in zip(arch_params, v_grads[:len(arch_params)]):
                dalpha.append(grad if grad is not None else torch.zeros_like(alpha))
            dw = v_grads[len(arch_params):]

            with torch.no_grad():
                for param, original in zip(weight_params, original_weights):
                    param.copy_(original)

            hessian = self._hessian_vector_product(
                (train_images, train_labels), weight_params, dw, bn_buffers
            )

            self.run_manager.net.zero_grad()
            for alpha, grad_alpha, hess in zip(arch_params, dalpha, hessian):
                alpha.grad = grad_alpha - current_lr * hess

            MixedEdge.MODE = 'darts'
            self.net.reset_binary_gates()
            expected_value = self._expected_hardware_value()
        finally:
            MixedEdge.MODE = None
            with torch.no_grad():
                for param, original in zip(weight_params, original_weights):
        ...
        return val_loss.data.item(), expected_value.item() if expected_value is not None else None
    ...
    def train(self):
        ...
        for epoch in range(self.run_manager.start_epoch, self.run_manager.run_config.n_epochs):
            ...
            for i, (images, labels) in enumerate(data_loader):
                ...
                if epoch > 0 and not self._use_darts_mode():
        ...
        ## print current network architecture
            ...
            for idx, block in enumerate(self.net.encoders):
                if block.operator is None:
                    self.write_log('%d. SKIPPED(None)' % idx, prefix='arch')
                elif block.skipped:
                    self.write_log('%d. SKIPPED(%s)' % (idx, block.operator.module_str), prefix='arch')
                else:
    ...
    def gradient_step(self, train_batch=None, current_lr=None):
        assert isinstance(self.arch_search_config, GradientArchSearchConfig)
        if self._use_darts_mode():
            return self._gradient_step_darts(train_batch=train_batch, current_lr=current_lr)
