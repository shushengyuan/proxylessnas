...
# candidates_type=whole 的基础候选；whole_all 在此基础上仅增加 Shuffle / Ghost（与 whole 其余完全一致）
WHOLE_CONV_CANDIDATES = [
    '3x3_MBConv2', '3x3_MBConv4', '5x5_MBConv6',
    '3x3_ResConv', '5x5_ResConv', '7x7_ResConv',
    '3x3_GroupConv2', '3x3_GroupConv4', '3x3_GroupConv8',
    '3x3_SpaConv', '5x5_SpaConv', '7x7_SpaConv',
]
WHOLE_ALL_SHUFFLE_GHOST_CANDIDATES = [
    '3x3_ShuffleMBConv2', '3x3_ShuffleMBConv4', '3x3_ShuffleMBConv8',
    '3x3_GhostMBConv2_r2', '5x5_GhostMBConv2_r2', '7x7_GhostMBConv2_r2',
]
...
def conv_name_define(conv_candidates, iterations):
    ...
    for m in range(int((1+iterations)*iterations/2)):
        if isinstance(conv_candidates, dict):
            conv_candidates_list = conv_candidates['iter_'+str(m)]
        else:
            conv_candidates_list = conv_candidates
        for i in range(len(conv_candidates_list)):
            op_name = conv_candidates_list[i]
            if op_name in darts_kernel_map:
                kernel = darts_kernel_map[op_name]
            else:
                kernel = op_name.split('x')[0]
            expand = 0
            group = 0
            if '_ShuffleMBConv' in op_name:
                expand = op_name.split('_ShuffleMBConv')[1]
            elif '_GroupConv' in op_name:
                group = op_name.split('_GroupConv')[1]
            elif '_GhostMBConv' in op_name:
                expand = op_name.split('_GhostMBConv')[1].split('_')[0]
            elif '_MBConv' in op_name:
                expand = op_name.split('_MBConv')[1]
            conv_name[op_name] = 'expand:' + str(expand) + "," + 'kernel:' + str(kernel) + "," + 'group:' + str(group)+ ","
...
def operation_choose(candidates_type):
    ...
    if candidates_type == 'whole':
        conv_candidates = list(WHOLE_CONV_CANDIDATES)

    elif candidates_type == 'whole_all':
        conv_candidates = list(WHOLE_CONV_CANDIDATES) + list(WHOLE_ALL_SHUFFLE_GHOST_CANDIDATES)

    elif candidates_type == 'Group_Spa':
        conv_candidates  = [ '3x3_GroupConv2', '3x3_GroupConv4', '3x3_GroupConv8',
                             '3x3_SpaConv',    '5x5_SpaConv',    '7x7_SpaConv']
    elif candidates_type == 'Group_Spa_Res':
        conv_candidates  = [ '3x3_GroupConv2', '3x3_GroupConv4', '3x3_GroupConv8',
                             '3x3_SpaConv',    '5x5_SpaConv',    '7x7_SpaConv',
                             '3x3_ResConv',    '5x5_ResConv',    '7x7_ResConv']
    elif candidates_type == 'Group_Spa_Res_MBConv':
        conv_candidates  = [ '3x3_GroupConv2', '3x3_GroupConv4', '3x3_GroupConv8',
                             '3x3_SpaConv',    '5x5_SpaConv',    '7x7_SpaConv',
                             '3x3_ResConv',    '5x5_ResConv',    '7x7_ResConv',
                             '3x3_MBConv2',    '3x3_MBConv4',    '5x5_MBConv6']
        ...
        conv_candidates  = [ '3x3_ResConv',    '5x5_ResConv',    '7x7_ResConv',
                             '3x3_GroupConv2', '3x3_GroupConv4', '3x3_GroupConv8',
                             '3x3_SpaConv',    '5x5_SpaConv',    '7x7_SpaConv']
    elif candidates_type == 'Res_Spa_MBConv':
        ...
        conv_candidates  = [ '3x3_MBConv2', '3x3_MBConv4', '3x3_MBConv6']
    elif candidates_type == 'MBConvOnly':
        # Keep the exact MBConv subset used inside Res_Group_Spa_MBConv so ablations
        # change only the non-MB operators, not the MBConv variants themselves.
        conv_candidates  = [ '3x3_MBConv2', '3x3_MBConv4', '5x5_MBConv6']
    ...
    elif candidates_type == 'Shuffle':
        conv_candidates  = ['3x3_ShuffleMBConv2', '3x3_ShuffleMBConv4', '3x3_ShuffleMBConv8']
    elif candidates_type == 'Ghost':
        conv_candidates  = ['3x3_GhostMBConv2_r2', '5x5_GhostMBConv2_r2', '7x7_GhostMBConv2_r2']
    elif candidates_type == 'Shuffle_Ghost':
        conv_candidates  = ['3x3_ShuffleMBConv2', '3x3_ShuffleMBConv4', '3x3_ShuffleMBConv8',
                            '3x3_GhostMBConv2_r2', '5x5_GhostMBConv2_r2', '7x7_GhostMBConv2_r2']
    elif candidates_type == 'DARTS':
        conv_candidates = ['max_pool_3x3', 'avg_pool_3x3', 'skip_connect',
...
def apply_final_conv_prior_bias(net, pi=0.0015):
    """Mask R-CNN-style prior bias on the last 1x1 conv: sigmoid(b)==pi mitigates early fg/bg imbalance.

    Same formula as BasicIRSTD_Freq/model/ResUNet/model_ResUNet_init.py lines 58-62.
    Call after ``net.apply(weights_init_xavier)`` so the final bias overwrites default / xavier state.
    """
    if pi is None or not (0.0 < pi < 1.0):
        return
    m = net.module if hasattr(net, 'module') else net
    block = getattr(m, 'post_transform_conv_block', None)
    if block is None:
        return
    for mod in block.modules():
        if isinstance(mod, nn.Conv2d) and mod.bias is not None:
            with torch.no_grad():
                mod.bias.fill_(-math.log((1.0 - pi) / pi))
            return
...
def decode(file, tot_iteration):
    ...
    # Gene files from older Phase1 searches may omit the deepest stage (e.g. enc_4_* when tot_iteration is 5).
    # Unfilled slots stay as [None] and break BFS; default to sequential connection [0].
    for i in range(tot_iteration):
        for j in range(len(codes[i])):
            c = codes[i][j]
            if c is None or (isinstance(c, list) and len(c) == 1 and c[0] is None):
                codes[i][j] = [0]
    print(codes)
    return codes
